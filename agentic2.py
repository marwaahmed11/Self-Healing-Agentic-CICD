...
# Direct Google GenAI SDK - no LangChain and no LangGraph
...
# ==========================================================
# Gemini - direct SDK, no LangChain
# ==========================================================
...
def find_callers(target_file: str) -> dict:
    ...
    for py_file in Path(".").rglob("*.py"):
        if any(
            part.startswith(".") or part in ("__pycache__", ".venv", "venv", "node_modules", "agentic_tmp")
...
# ==========================================================
# State - same concept as the original, but no LangGraph
# ==========================================================
...
# ==========================================================
# Nodes - kept as normal Python functions
# ==========================================================
...
def fix_code_node(state: AgenticState) -> dict:
    ...
    log(
        "INFO",
        "NODE[fix_code_node]: Generating surgical patch for %s",
        current_file,
    )
...
def test_code_node(state: AgenticState) -> dict:
    ...
    log("INFO", "NODE[test_code_node]: Testing ALL accumulated patches...")
...
def run_agent() -> AgenticState:
    ...
    # Initial log fetch.
    state.update(fetch_logs_node(state))
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # Analyze failure.
        try:
            state.update(analyze_code_node(state))
        except Exception as e:
        ...
        fix_result = fix_code_node(state)
        ...
        # Syntax validation.
        lint_result = lint_check_node(state)
        ...
        # Real tests.
        test_result = test_code_node(state)
    ...
    rca_result = generate_rca_node(state)
    state.update(rca_result)

    # Create PR only after successful tests.
    if state.get("success"):
        pr_result = create_pr_node(state)
